...
class Approach(Base):
    ...
    expires_at: Mapped[datetime] = mapped_column(DateTime, default=_expires_at_default)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    responded_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=None)
...
class UserCandidate(Base):
    ...
    def __init__(self, **kwargs):
        ...
        if 'scored_at' not in kwargs:
            kwargs['scored_at'] = datetime.utcnow()
        super().__init__(**kwargs)
